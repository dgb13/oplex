...
export interface CatalogCardRow {
  ...
  totalStock: number;
  active: boolean;
  // Fabricados y servicios no se compran - sin "Agregar" al carrito (mismo
  // criterio que AddToCartIconButton en la vista tabla).
  isService: boolean;
...
function ArticleCard({
  ...
  const addItem = useMutation({
    mutationFn: () =>
      cartApi.addItem({ articleVariantId: row.variantId, quantity }),
    onSuccess: () => {
    ...
    <Card className={`overflow-hidden py-0 ${!row.active ? 'opacity-50' : ''}`}>
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        <div className="flex flex-wrap items-center gap-1">
          ...
          {!row.active && (
            <Badge className="w-fit bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300">
              Inactivo
            </Badge>
          )}
        </div>
